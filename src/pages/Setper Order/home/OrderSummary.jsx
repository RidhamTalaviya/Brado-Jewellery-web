// OrderSummary component (modified with back button and button text change)
...
const OrderSummary = ({ nextStep, setShowCouponModal , currentStep, prevStep   }) => {
  ...
  console.log(appliedCoupon , "appliedCoupon ORDER summery");
    ...
    <div className="bg-white rounded-lg p-6 h-fit">
      ...
      {appliedCoupon && (
        <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg flex items-center justify-between">
          ...
          <button
            ...
            onClick={() => {
              // Add logic to remove coupon here
      ...
      <div className="pt-4">
        <h3 className="mb-4 text-[16px]">
          Order Summary <span className="text-[#696661] text-[14px]">(items {cart?.products?.length || 0})</span>
        </h3>
        ...
        {currentStep > 1 && prevStep && (
          <button onClick={prevStep} className="w-full bg-white border border-gray-300 py-3 mb-4 rounded">
            Back
          </button>
        )}
        <button onClick={nextStep} className="w-full bg-[#b4853e] text-white py-3 mb-4 rounded">
